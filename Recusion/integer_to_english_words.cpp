// Integer to English Words (Hard problem of Leetcode)
#include<bits/stdc++.h>
using namespace std;
...
string solve(vector<pair<int,string>> & mp,int num){
    if(num == 0)
        return "Zero";
        ...
        for(auto it:mp){
            if(num >= it.first){
                string a = "";
                if(num >= 100){
                    a = solve(mp, num / it.first) + " ";
                }
                string b = it.second;
                string c = "";
                if( num % it.first != 0){
                    c =" "+ solve(mp,num % it.first);
                }
                return a + b + c;
            }
        }
